      ...
      <div className="rounded-[24px]  p-4 ">
        ...
        <nav className="space-y-2">
          {navItems.map(({ label, icon: Icon, active }) => (
            <button
              key={label}
              className={`flex w-full items-center justify-start gap-5 rounded-[14px] px-3 py-3 text-right text-sm font-medium transition ${active ? 'bg-[white]/10 hover:bg-white/20 text-white' : 'bg-transparent text-white/80 hover:bg-white/10'}`}
            >
              <Icon size={18} />
              <span>{label}</span>
